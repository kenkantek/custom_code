...
//			}
	...
	function getFormAddPicture(idRecord)
	{
		$('#placeHolder').hide();

		callAjax('POST','ajaxLoading',
					processFolder + 'get_form_add_picture.php',
					'id=' + idRecord,
					beforeSend,
					getFormAddPictureSucess,
					postDataError);

		return false;
	}

	function getFormAddPictureSucess(returnData)
	{
		...
		$('#frmData').submit(function(){
			...
			//callsubmit
			return submitFormAddPicture();
		});

		//get list picture
		listPicture();

		$('a.reloadPictureList').click(function(){
			listPicture();
			return false;
		})
	}
	...
	function submitFormAddPicture()
	{
		$('#placeHolder').hide();

		$('#frmData').ajaxSubmit({
					beforeSubmit:	beforeSendForm,
					success:		doAddPictureSucess,
					url:			processFolder + 'do_add_picture.php',
					dataType:		'json',
					error:			postDataError
		});
	...
	function doAddPictureSucess(returnData)
	{
		$('#ajaxLoading').html('').hide();
	...
	{
		$('#placeHolderListPicture').hide();

		callAjax('POST','ajaxLoading',
					processFolder + 'get_list_picture.php',
					'id=' + $('#id').val() + '&page=' + listPictureCurrentPage,
					beforeSend,
					listPictureSuccess,
					listPictureError);
	...
	function listPictureSuccess(returnData)
	{
		$('#ajaxLoading').html('').hide();

		if(returnData.result)
		{
			$('#placeHolderListPicture').show();
			$('#placeHolderListPicture').html(returnData.data);
		}
		else
		{
			displayMessageBox(returnData.data);
		...
		$("a.paging").click(function(){
			listPictureCurrentPage = $(this).attr('rel');
			listPicture();
		})
		...
		$(".delete").jConfirmAction({'yesFunction' : doDeletePicture});

		$(".changeStatus").jConfirmAction({'yesFunction' : doChangeStatusPicture});

	}

	function doDeletePicture(idRecord)
	{
		$('#placeHolderListPicture').hide();

		callAjax('POST','ajaxLoading',
			processFolder + 'do_delete_picture.php',
			'id=' + idRecord,
			beforeDelete,
			doDeletePictureSucess,
			postDataPictureError);

	}

	function doDeletePictureSucess(returnData)
	{
		if(!returnData.result)
		{
			alert(returnData.data);
	...
	function doChangeStatusPicture(param)
	{
		$('#placeHolderListPicture').hide();

		callAjax('POST','ajaxLoading',
			processFolder + 'do_change_status_picture.php',
			'param=' + param,
			beforeChangeStatus,
			doChangeStatusPictureSucess,
			postDataPictureError);

	}

	function doChangeStatusPictureSucess(returnData)
	{
		if(!returnData.result)
		{
			alert(returnData.data);
	...
	function listPictureError(data)
	{
		$('#placeHolderListPicture').html('Cannot get picture list. Please refresh and retry');
	}

	function postDataPictureError(data)
	{
		$('#placeHolderListPicture').html('Cannot post data. Please refresh and retry');
	}
